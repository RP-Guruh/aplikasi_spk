class GenerateReportController < ApplicationController
  ...
  def employee_report
    ...
    send_data pdf, filename: 'employee.pdf', type: 'application/pdf'
  end
  ...
  def hasil_report
    ...
    pdf = Grover.new(html).to_pdf
    send_data pdf, filename: 'hasil_report.pdf', type: 'application/pdf'
  end
  ...
  def normalisasi_bobot
    ...
    pdf = Grover.new(html).to_pdf
    send_data pdf, filename: 'normalisasi_bobot.pdf', type: 'application/pdf'
  end
  ...
  def normalisasi_report
    ...
    pdf = Grover.new(html).to_pdf
    send_data pdf, filename: 'normalisasi_report.pdf', type: 'application/pdf'
  end
end
